...
std::vector<duckdb_libpgquery::PGSimplifiedToken> PostgresParser::Tokenize(
...
duckdb_libpgquery::PGKeywordCategory PostgresParser::IsKeyword(
    const std::string &text) {
  return duckdb_libpgquery::is_keyword(text.c_str());
}

std::vector<duckdb_libpgquery::PGKeyword> PostgresParser::KeywordList() {
  // FIXME: because of this, we might need to change the libpg_query library to
  // use duckdb::vector
  std::vector<duckdb_libpgquery::PGKeyword> tmp(
      duckdb_libpgquery::keyword_list());
  return tmp;
}
