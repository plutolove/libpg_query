 ...
 *	CTE_SUBLINK			WITH query (never actually part of an
 *expression) For ALL, ANY, and ROWCOMPARE, the lefthand is a list of
 *expressions of the same length as the subselect's targetlist.  ROWCOMPARE will
 **always* have a list with more than one entry; if the subselect has just one
 *target then the parser will create an EXPR_SUBLINK instead (and any operator
